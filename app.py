...
def load():
    ...
    with open(hot100, mode='r') as file:
        ...
        for lines in csvFile:
            ...
            if year > 2000:
                continue   # ignore the new records for now.
            ...
            try:
                ...
                if record is None:
                    ...
                    record_to_insert = (lines[sidIn], lines[sIn], lines[pIn], int(
                        lines[ppIn]), int(lines[iIn]), int(lines[wocIn]), year)
                    ...
                    # save the top position of the song in the main record
                    if top_position > int(lines[ppIn]):
                        top_position = int(lines[ppIn])
                        updated = True
                    # save the highest number of weeks on the chart
                    ...
                    if updated:
                        ...
                        cursor.execute(
                            sql_update_query, (top_position, weeksonchart, chartyear, id))

                # write to other tables

                sql_query = "select id from weeks where weekinfo = '" + \
                    str(lines[wIn]) + "'"
                ...
                if record is None:
                    ...
                    postgres_insert_query = "INSERT INTO weeks (weekinfo, weekdate, year) VALUES ('" + str(
                        lines[wIn]) + "', '" + str(lines[wIn]) + "' , " + str(year) + ")"
                ...
                if record is None:
                    ...
                    record_to_insert = (id, weekid, lines[uIn], int(
                        lines[wpIn]), int(lines[ppIn]))
                    cursor.execute(postgres_insert_query, record_to_insert)
                ...
                for performer in mperf:
                    ...
                    sql_query = "select id from performer where performer = '" + \
                        performer + "' and id = " + str(id)

                    cursor.execute(sql_query)
                    record = cursor.fetchone()
                    if record == None:

                        postgres_insert_query = "INSERT INTO performer (id, performer) VALUES ('" + str(
                            id) + "', '" + performer + "')"
                        cursor.execute(postgres_insert_query)
    ...
    with open(spotInfo, mode='r', encoding='utf-8') as file:
        ...
        for lines in csvFile:
            ...
            try:
                ...
                sql_query = "select id from song where songid = '" + \
                    lines[0] + "';"
                ...
                if record is None:
                    continue  # This record has not been pulled into the database
                ...
                sql_query = "select EXISTS(select id from stats where id = " + \
                    str(id) + ");"
                ...
                if record[0]:
                    continue  # This record has been saved into the database
...
def searchSplit(result):
    ...
    for res in result:
        ...
        if srchkey[1] != 'All':
            result += "and " + lower + \
                srchkey[0] + lower2 + " = " + lower + "'" + \
                check_string(srchkey[1]) + "'" + lower2
    return result
...
def get_top100_sql_performer(performer='*'):
    ...
    try:
        ...
        if performer != '*':
            ...
            performer = check_string(performer)
            sql_query = "select name, s.performer, top_position, chartyear, weeksonchart from song as s inner join performer as p on s.id = p.id where lower(p.performer) = lower('" + \
                performer + "')"
        else:
...
def get_top100_sql_song_details(song):
    ...
    try:
        # need to add a single quote where necessary for query
        song = check_string(song)
        ...
        sql_query += " inner join weeks wk on w.weekid = wk.id where lower(name) = lower('" + \
            song + "')"
...
def get_top100_sql_select(selectInput):
    ...
    if len(result) == 1:
        ...
        sql_order = " order by top_position asc, chartyear, weeksonchart desc OFFSET " + \
            result[3].split('=')[1] + " limit 100"
...
def get_top100_sql_partialsearch(searchInput):
    ...
    sql_query_where = "where lower(p.performer) like '%" + \
        result + "%' or lower(name) like '%" + result + "%'"
...
def test(track_ids, playlistName):
    ...
    if playlistName == '':
        playlistName = 'Top100 Billboard PlayList'
    ...
    try:
        ...
        if playlistId == '':
            playlistInfo = sp.user_playlist_create(
                user, playlistName, public=True, collaborative=False, description='Created from Top100 songs')
            playlistId = playlistInfo['id']

        playlistInfo = sp.user_playlist_replace_tracks(
            user, playlistId, track_ids)
